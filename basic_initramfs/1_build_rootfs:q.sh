...
# bit of a hack
pacman -Sy --noconfirm -r /root base ${install_packages[@]}
FOE
}
function remote_root_password_in_rootfs {
    sudo chroot $mount_point /bin/sh <<EOF
passwd -d root
EOF
}
...
function mount_virtual_filesystems {
    ...
    sudo mount -o bind /run $mount_point/run
}

function unmount_virtual_filesystems {
